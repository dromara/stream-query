...
public class DefaultDataSourceDestroyer implements DataSourceDestroyer {
  ...
  public void asyncDestroy(String name, DataSource dataSource) {
    log.info(
        "dynamic-datasource start asynchronous task to close the datasource named [{}],", name);
  ...
  private void graceDestroy(String name, DataSource dataSource) {
    try {
      DataSourceActiveDetector detector =
          detectors.stream().filter(x -> x.support(dataSource)).findFirst().orElse(null);
      ...
      log.warn(
          "dynamic-datasource check the datasource named [{}] contains active connection failed,",
          name,
          e);
    }
