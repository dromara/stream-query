...
import org.dromara.streamquery.stream.plugin.mybatisplus.engine.dynamicdatasource.datasource.exception.ErrorCreateDataSourceException;
...
public class DruidDataSourceCreator implements DataSourceCreator {
  ...
  /**
   * Druid since 1.2.17 use 'configFromPropeties' to copy config Druid < 1.2.17 use
   * 'configFromPropety' to copy config
   */
  private static void fetchMethod() {
    ...
    throw new RuntimeException(
        "Druid does not has 'configFromPropeties' or 'configFromPropety' method!");
  }
    ...
    if (DsStrUtils.hasText(filters)) {
      ...
      for (String filter : filterItems) {
        switch (filter) {
          ...
          case "slf4j":
            proxyFilters.add(
                DruidLogConfigUtil.initFilter(Slf4jLogFilter.class, config.getSlf4j()));
            break;
          case "commons-log":
            proxyFilters.add(
                DruidLogConfigUtil.initFilter(CommonsLogFilter.class, config.getCommonsLog()));
            break;
          case "log4j":
