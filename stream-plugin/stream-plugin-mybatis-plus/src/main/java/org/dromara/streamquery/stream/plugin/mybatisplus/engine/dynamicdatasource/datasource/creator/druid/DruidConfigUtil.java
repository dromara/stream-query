 ...
 */
package org.dromara.streamquery.stream.plugin.mybatisplus.engine.dynamicdatasource.datasource.creator.druid;

import com.alibaba.druid.pool.DruidDataSource;
import lombok.NonNull;
import lombok.SneakyThrows;
import lombok.extern.slf4j.Slf4j;
import org.dromara.streamquery.stream.plugin.mybatisplus.engine.dynamicdatasource.datasource.toolkit.DsConfigUtil;
...
public final class DruidConfigUtil {
  ...
  private static final Map<String, PropertyDescriptor> CONFIG_DESCRIPTOR_MAP =
      DsConfigUtil.getPropertyDescriptorMap(DruidConfig.class);
  private static final Map<String, PropertyDescriptor> DATASOURCE_DESCRIPTOR_MAP =
      DsConfigUtil.getPropertyDescriptorMap(DruidDataSource.class);
  ...
  public static Properties toProperties(@NonNull DruidConfig config) {
    ...
    Properties connectProperties =
        Optional.ofNullable(config.getConnectionProperties()).orElse(new Properties());
  ...
  private static void mergeList(DruidConfig global, DruidConfig item, PropertyDescriptor pd) {
    ...
    List<Object> itemValue = (List<Object>) readMethod.invoke(item);
    List<Object> globalValue = (List<Object>) readMethod.invoke(global);
    if (globalValue != null) {
