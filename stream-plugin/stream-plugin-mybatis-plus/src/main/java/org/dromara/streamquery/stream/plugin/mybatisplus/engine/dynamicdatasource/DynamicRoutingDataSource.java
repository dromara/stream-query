...
import org.dromara.streamquery.stream.plugin.mybatisplus.engine.dynamicdatasource.datasource.destroyer.DataSourceDestroyer;
import org.dromara.streamquery.stream.plugin.mybatisplus.engine.dynamicdatasource.datasource.destroyer.DefaultDataSourceDestroyer;
import org.dromara.streamquery.stream.plugin.mybatisplus.engine.dynamicdatasource.datasource.exception.CannotFindDataSourceException;
import org.dromara.streamquery.stream.plugin.mybatisplus.engine.dynamicdatasource.datasource.toolkit.DsStrUtils;
import org.dromara.streamquery.stream.plugin.mybatisplus.engine.dynamicdatasource.ds.AbstractRoutingDataSource;
import org.dromara.streamquery.stream.plugin.mybatisplus.engine.dynamicdatasource.ds.ItemDataSource;
...
public class DynamicRoutingDataSource extends AbstractRoutingDataSource
  ...
  private final DataSourceProperty defaultDbProperty;
  ...
  @Setter private Boolean graceDestroy = false;

  public DynamicRoutingDataSource(
      DefaultDataSourceCreator defaultDataSourceCreator, DataSourceProperty properties) {
    dataSourceCreator = defaultDataSourceCreator;
    defaultDbProperty = properties;
  }
  ...
  public DataSource getDataSource(String ds) {
    ...
    if (strict) {
      throw new CannotFindDataSourceException(
          "dynamic-datasource could not find a datasource named " + ds);
    }
    return determinePrimaryDataSource();
  }
  ...
  public void afterPropertiesSet() {
    ...
    //        checkEnv();
    // 添加并分组数据源
    Map<String, DataSource> dataSources = Maps.of();
    DataSource defaultDataSource = dataSourceCreator.createDataSource(defaultDbProperty);
    DynamicDataSourceContextHolder.push(primary);
    dataSources.put(primary, defaultDataSource);
