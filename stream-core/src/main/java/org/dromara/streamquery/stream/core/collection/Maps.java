...
public class Maps {
     ...
     * @return 合并后的map
     */
    public static <K, V> Map<K, V> merge(Map<K, V> m1, Map<K, V> m2, BiFunction<V, V, V> mergeFunction) {
     ...
     * @param <V> a V object
     */
    public static <K, V> Map<K, List<V>> filterByListValue(Map<K, List<V>> map, Predicate<List<V>> predicate) {
        map.entrySet().removeIf(entry -> !predicate.test(entry.getValue()));

        // 将符合条件的entry重新放入新的Map中返回
        return Steam.of(map.entrySet())
                .filter(entry -> !entry.getValue().isEmpty()).toMap(
                        Map.Entry::getKey,
                        Map.Entry::getValue
                );
    }
    ...
    public static <K, V> Map<String, V> flatten(Map<K, Map<K, V>> nestedMap, String delimiter) {
        return Steam.of(nestedMap.entrySet())
                .flatMap(entry -> Steam.of(entry.getValue().entrySet())
                        .map(innerEntry -> new AbstractMap.SimpleEntry<>(
                                entry.getKey() + delimiter + innerEntry.getKey(), innerEntry.getValue())))
