...
public class Collectors {
    ...
    Collector<T, ?, C> toCollection(Supplier<C> collectionFactory) {
        return new Collectors.CollectorImpl<>(collectionFactory, Collection::add,
                (r1, r2) -> {
                    r1.addAll(r2);
                    return r1;
                },
    ...
    public static <K, V> Collector<Map<K, V>, Map<K, Collection<V>>, Map<K, Collection<V>>> mapMerger(
            Supplier<Map<K, Collection<V>>> mapSupplier,
            Supplier<Collection<V>> listSupplier
    ) {
        return mapMerger(mapSupplier, listSupplier, Function.identity());
    }

    public static <K, V> Collector<Map<K, V>, Map<K, Collection<V>>, Map<K, Collection<V>>> mapMerger() {
        return mapMerger(ConcurrentHashMap<K, Collection<V>>::new, ArrayList<V>::new);
    }

    public static <K, V, R> Collector<Map<K, V>, Map<K, Collection<V>>, Map<K, R>> mapMerger(
            Supplier<Map<K, R>> mapSupplier,
            Supplier<Collection<V>> listSupplier,
            Function<Collection<V>, R> downstream
    ) {

        return new Collector<Map<K, V>, Map<K, Collection<V>>, Map<K, R>>() {
            @Override
            ...
            @Override
            public BiConsumer<Map<K, Collection<V>>, Map<K, V>> accumulator() {
                return (kListMap, kvMap) -> kvMap.forEach((k, v) -> {
                    Collection<V> vList = kListMap.get(k);
                    if (vList == null) {
                        vList = listSupplier.get();
                    }
                    vList.add(v);
                    kListMap.put(k, vList);
                });
            }
            ...
            public BinaryOperator<Map<K, Collection<V>>> combiner() {
                return (kListMap, kListMap2) -> {
                    kListMap2.forEach(
                            (k, vList) ->
                                    kListMap.merge(k, vList, (vList1, vList2) -> {
            ...
            @Override
            public Function<Map<K, Collection<V>>, Map<K, R>> finisher() {
                return m -> {
                    Map<K, R> map = mapSupplier.get();
                    m.forEach((k, vs) -> {
                        R apply = downstream.apply(vs);
                        map.put(k, apply);
                    });
                    return map;
                };
    ...
    public static <K, V, R> Collector<Map<K, V>, Map<K, Collection<V>>, Map<K, R>> mapMerger(
        ...
        return mapMerger(ConcurrentHashMap<K, R>::new, ArrayList<V>::new, downstream);
    }
    ...
    Collector<T, ?, Map<Boolean, D>> partitioningBy(Predicate<? super T> predicate,
                                                    Collector<? super T, A, D> downstream) {
        BiConsumer<A, ? super T> downstreamAccumulator = downstream.accumulator();
        BiConsumer<Collectors.Partition<A>, T> accumulator = (result, t) ->
                downstreamAccumulator.accept(predicate.test(t) ? result.forTrue : result.forFalse, t);
        BinaryOperator<A> op = downstream.combiner();
        BinaryOperator<Collectors.Partition<A>> merger = (left, right) ->
                new Collectors.Partition<>(op.apply(left.forTrue, right.forTrue),
                        op.apply(left.forFalse, right.forFalse));
        Supplier<Collectors.Partition<A>> supplier = () ->
                new Collectors.Partition<>(downstream.supplier().get(),
                        downstream.supplier().get());
        if (downstream.characteristics().contains(Collector.Characteristics.IDENTITY_FINISH)) {
            return new Collectors.CollectorImpl<>(supplier, accumulator, merger, CH_ID);
        } else {
            Function<Collectors.Partition<A>, Map<Boolean, D>> finisher = par ->
                    new Collectors.Partition<>(downstream.finisher().apply(par.forTrue),
    ...
    Collector<T, ?, M> toConcurrentMap(Function<? super T, ? extends K> keyMapper,
        ...
        return new Collectors.CollectorImpl<>(mapSupplier, accumulator, mapMerger(mergeFunction), CH_CONCURRENT_ID);
    }
    ...
    private static final class Partition<T>
        ...
        public boolean equals(Object o) {
            ...
            return Objects.equals(forTrue, partition.forTrue) && Objects.equals(forFalse, partition.forFalse);
        }
