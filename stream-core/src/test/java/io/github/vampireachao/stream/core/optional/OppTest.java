...
class OppTest {
    ...
    void getTest() {
        ...
        final Object opp = Opp.ofNullable(null).get();
    ...
    void peekTest() {
        ...
        // 相当于ifPresent的链式调用
        Opp.ofNullable("hutool").peek(user::setUsername).peek(user::setNickname);
        ...
        final String name = Opp.ofNullable("hutool").peek(username -> username = "123").peek(username -> username = "456").get();
    ...
    void peeksTest() {
        ...
        // 可以一行搞定
        Opp.ofNullable("hutool").peeks(user::setUsername, user::setNickname);
        ...
        final String name = Opp.ofNullable("hutool").peeks(
                username -> username = "123", username -> username = "456",
                n -> Assertions.assertEquals("hutool", n)).get();
        Assertions.assertEquals("hutool", name);

        // 当然，以下情况不会抛出NPE，但也没什么意义
        Opp.ofNullable("hutool").peeks().peeks().peeks();
        Opp.ofNullable(null).peeks(i -> {
        });
    ...
    void orTest() {
        ...
        // 给一个替代的Opp
        final String str = Opp.<String>ofNullable(null).or(() -> Opp.ofNullable("Hello hutool!")).map(String::toUpperCase).orElseThrow();
        Assertions.assertEquals("HELLO HUTOOL!", str);

        final User user = User.builder().username("hutool").build();
        final Opp<User> userOpp = Opp.of(user);
        // 获取昵称，获取不到则获取用户名
...
//		Opp.ofNullable(userList).map(List::stream).flatMap(Stream::findFirst);
        // 现在，兼容
        final User user = Opp.ofNullable(userList).map(List::stream)
    ...
    void emptyTest() {
        ...
        final List<String> past = Opp.ofNullable(Collections.<String>emptyList()).filter(l -> !l.isEmpty()).orElseGet(() -> Collections.singletonList("hutool"));
        // 现在，一个empty搞定
